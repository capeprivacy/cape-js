...
describe('Cape', () => {
  ...
  describe('#invoke', () => {
    ...
    test('when the server responds with an unknown message, it should throw an error and disconnect', async () => {
      const id = 'ABC';
      const capeApiUrl = 'http://localhost:1282';
      const enclaveUrl = 'ws://localhost:1282';
...
-----END PUBLIC KEY-----`;
    ...
    test('should fetch a key by username without error and cache it', async () => {
      ...
      const client = new Cape({ authToken, capeApiUrl, enclaveUrl, checkDate: keyCheckDate });
      const result = await client.key('cool-user');
      const expected = `-----BEGIN PUBLIC KEY-----
MIICIjANBgkqhkiG9w0BAQEFAAOCAg8AMIICCgKCAgEAnV8eokFPI6Nx+MJ+4iBG
+5Ms1W2qadbuKf8pFL3s8aEsLp4MSH7809b8nsLhgxtbs+nBh1CNaoOxkxY2bhhH
Kv8gmnkP0kuH0dRKz9Egf0R3CV+vA+lnRnZThoz8GeouXCmRoYT/crfIjNq4FnKb
...
-----END PUBLIC KEY-----`;
  ...
  test('when the server responds with an error, it should throw the error', async () => {
    const id = 'ABC';
    const capeApiUrl = 'http://localhost:8122';
    const enclaveUrl = 'ws://localhost:8122';
    const mockServer = new Server(`${enclaveUrl}/v1/run/${id}`);
